...
#undef S
  ...
  Score Evaluation<T>::king() const {
    ...
    kingDanger +=        kingAttackersCount[Them] * kingAttackersWeight[Them]
                 ...
                 + 149 * popcount(unsafeChecks)
                 +  94 * popcount(pos.blockers_for_king(Us))
                 +  61 * kingAttacksCount[Them]
                 +   3 * kingFlankAttacks * kingFlankAttacks / 8
                 +       mg_value(mobility[Them] - mobility[Us])
                 - 867 * !pos.count<QUEEN>(Them)
                 - 106 * bool(attackedBy[Us][KNIGHT] & attackedBy[Us][KING])
                 -  38 * bool(attackedBy[Us][BISHOP] & attackedBy[Us][KING])
                 -   6 * mg_value(score) / 8
                 -   6;
