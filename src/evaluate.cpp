  ...
  Value Evaluation<T>::winnable(Score score) const {
    ...
    if (sf == SCALE_FACTOR_NORMAL)
    {
        if (pos.opposite_bishops())
        {
            ...
            {
                if ( strongSide == WHITE )
                    sf = 18 + 4 * popcount(shift<NORTH>(pe->passed_pawns(WHITE)) & ~pos.pieces() & ~attackedBy2[BLACK]);
                else
                    sf = 18 + 4 * popcount(shift<SOUTH>(pe->passed_pawns(BLACK)) & ~pos.pieces() & ~attackedBy2[WHITE]);
            }
            else
