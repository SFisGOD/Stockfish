        ...
        if (pos.opposite_bishops())
        {
            if (   pos.non_pawn_material(WHITE) == BishopValueMg
                ...
                sf = 22 + 3 * pos.count<ALL_PIECES>(strongSide);
                 ...
                 && pos.non_pawn_material(BLACK) == BishopValueMg)
            sf = std::min(sf, 18 + 7 * pos.count<PAWN>(strongSide) 
                                 +10 * pos.pawns_on_same_color_squares(~strongSide, pos.square<BISHOP>(strongSide))
                                 - 7 * (pos.count<PAWN>(~strongSide) - pos.pawns_on_same_color_squares(~strongSide, pos.square<BISHOP>(strongSide))));
