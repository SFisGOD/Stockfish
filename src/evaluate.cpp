...
Value Eval::evaluate(const Position& pos) {
  ...
  Value psqt = Value(eg_value(pos.psq_score()));
  int node = pos.this_thread()->nodes;
  bool useClassical = psqt * 16 > NNUEThreshold1 * (16 + pos.rule50_count());
  bool classical = !Eval::useNNUE
                ||  useClassical
                || (psqt > PawnValueMg / 4 && !(node & 0xB))                              // 1/8  chance
                || (!(psqt > PawnValueMg / 4) && psqt > PawnValueMg / 8 && !(node & 0xF)) // 1/16 chance
                || (!(psqt > PawnValueMg / 8) && !(node & 0x1F));                         // 1/32 chance
