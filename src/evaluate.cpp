...
Value Eval::evaluate(const Position& pos) {
  ...
  else
  {
      ...
      if (largePsq)
      {
          ...
          // For single bishop endgames, switch to NNUE with 50% probability
          // if classical eval is not yet in win zone.
          else if (   pos.non_pawn_material() == BishopValueMg
                   && abs(v) < NNUEThreshold1
                   && pos.this_thread()->nodes & 0x1)
              v = adjusted_NNUE();

          // For the case of opposite colored bishops, switch to NNUE with small
