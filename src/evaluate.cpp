...
namespace {
  ...
  constexpr Value NNUEThreshold3 =   Value(950);
...
Value Eval::evaluate(const Position& pos) {
  ...
  else
  {
      ...
      // For the case of opposite colored bishops and queen imbalance, switch to NNUE
      // eval with small probability if the classical eval is less than the threshold.
      if (largePsq)
      {
          if (abs(v) * 16 < NNUEThreshold2 * r50)
              ...
              v = adjusted_NNUE();
          else if (   !(pos.this_thread()->nodes & 0x13)
                   && pos.count<QUEEN>() == 1
                   && abs(v) * 16 < (NNUEThreshold3 + pos.non_pawn_material() / 64) * r50)
              v = adjusted_NNUE();
      }
  }
