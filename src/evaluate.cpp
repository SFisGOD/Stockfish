...
Value Eval::evaluate(const Position& pos) {
  ...
  else
  {
      ...
      bool  classical =   (largePsq && (pos.this_thread()->nodes & 0xAA))                // 15 in 16 chance
                       || (!(pos.this_thread()->nodes & 0x15) && psq > PawnValueMg / 4); //  1 in  8 chance

      v = classical ? Evaluation<NO_TRACE>(pos).value() : adjusted_NNUE();

      // If the classical eval is small and imbalance large, use NNUE nevertheless.
      if (largePsq && (pos.this_thread()->nodes & 0xAA))
      {
