  ...
  ScaleFactor Evaluation<T>::scale_factor(Value eg) const {
    ...
    if (sf == SCALE_FACTOR_NORMAL)
    {
        if (   pos.opposite_bishops()
            ...
            sf = 4 * (pe->pawn_asymmetry() + pos.count<PAWN>(strongSide) - pos.count<PAWN>(~strongSide));
        else
            sf = std::min(35 + (pos.opposite_bishops() ? 3 : 6) * (pe->pawn_asymmetry() + pos.count<PAWN>(strongSide)), sf);
