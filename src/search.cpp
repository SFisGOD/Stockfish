...
namespace {
  ...
  Value value_draw(Score psq, Thread* thisThread) {
    return VALUE_DRAW + Value(  (abs(eg_value(psq)) > PawnValueMg / 16 ? 3 : 2)
                              * (thisThread->nodes & 1) 
                              - (abs(eg_value(psq)) > PawnValueMg / 16 ? 0 : 1));
  }
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        alpha = value_draw(pos.psq_score(), pos.this_thread());
        if (alpha >= beta)
            return alpha;
    }
    ...
    if (!rootNode)
    {
        ...
        if (   Threads.stop.load(std::memory_order_relaxed)
            ...
            return (ss->ply >= MAX_PLY && !ss->inCheck) ? evaluate(pos)
                                                        : value_draw(pos.psq_score(), pos.this_thread());
    ...
    else if (ss->ttHit)
    {
        ...
        if (eval == VALUE_DRAW)
            eval = value_draw(pos.psq_score(), thisThread);
